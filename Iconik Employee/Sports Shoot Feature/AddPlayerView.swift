...
struct AddPlayerView: View {
    ...
    private func loadSportsShoot() {
        // Load the sports shoot to get school and sport names for searchable fields
        SportsShootService.shared.fetchSportsShoot(id: shootID, loadPlayers: false, loadGroups: false) { result in
            switch result {
    ...
    private func savePlayer() {
        ...
        let player = Player(
            id: existingPlayer?.id ?? UUID().uuidString,
            jobId: shootID,
            ...
            searchableSchool: sportsShoot?.schoolName ?? "",
            searchableSport: sportsShoot?.sportName ?? ""
        )
        
        if isEditing {
            SportsShootService.shared.updatePlayer(player) { result in
            ...
            SportsShootService.shared.addPlayer(player) { result in
                DispatchQueue.main.async {
                    self.isLoading = false
