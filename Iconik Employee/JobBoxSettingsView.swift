//
//  JobBoxStalledTimings.swift
//  Iconik Employee
//
//  Created by administrator on 5/11/25.
//
...
struct JobBoxSettingsView: View {
    ...
    init() {
        ...
        _packedHours = State(initialValue: settingsManager.stalledTimings.packed)
        _pickedUpHours = State(initialValue: settingsManager.stalledTimings.pickedUp)
        _leftJobHours = State(initialValue: settingsManager.stalledTimings.leftJob)
    }
        ...
        Form {
            Section(header: Text("Stalled Time Thresholds").font(.headline)) {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    HStack {
                        Slider(value: $packedHours, in: 0.5...12, step: 0.5)
                            .onChange(of: packedHours) { newValue in
                                updateSettings()
                            }
                        
                        Text("\(packedHours, specifier: "%.1f") hrs")
                            .frame(width: 60)
                    }
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    HStack {
                        Slider(value: $pickedUpHours, in: 0.5...24, step: 0.5)
                            .onChange(of: pickedUpHours) { newValue in
                                updateSettings()
                            }
                        
                        Text("\(pickedUpHours, specifier: "%.1f") hrs")
                            .frame(width: 60)
                    }
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    HStack {
                        Slider(value: $leftJobHours, in: 0.5...48, step: 0.5)
                            .onChange(of: leftJobHours) { newValue in
                                updateSettings()
                            }
