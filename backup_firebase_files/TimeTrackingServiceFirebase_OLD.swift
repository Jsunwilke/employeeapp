    ...
    func clockIn(sessionId: String? = nil, notes: String? = nil, completion: @escaping (Bool, String?) -> Void) {
        ...
        getCurrentTimeEntry(userId: userId, organizationID: orgId) { [weak self] activeEntry in
            ...
            if let sessionId = sessionId {
                ...
                // Fetch session name using async/await
                Task {
                    var sessionName = "Unknown Session"
                    do {
                        if let session = try await SessionService.shared.fetchSession(sessionId: sessionId) {
                            sessionName = await SessionService.shared.getSessionDisplayName(for: session)
                        } else {
                            print("⚠️ Session not found for ID: \(sessionId)")
                        }
                    } catch {
    ...
    func getTodayAssignedSessions(completion: @escaping ([Session]) -> Void) {
        ...
        Task { @MainActor in
            do {
                ...
                let allSessions = try await SessionService.shared.fetchSessions(organizationID: orgId, includeUnpublished: false)

                // Filter for today and user-assigned sessions
        ...
        Task { @MainActor in
            do {
                ...
                let filteredSessions = allSessions.filter { session in
                    ...
                    // Note: hasJobBoxAssigned is not in the Session model
                    // This filter has been removed - may need to be handled differently
                }

                // Sort by date and time
                let sortedSessions = filteredSessions.sorted { (session1, session2) in
