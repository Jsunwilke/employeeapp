...
- [ ] **Shared RosterEditingController** (highest-value refactor): `FPSportsRosterView_iPad.swift` (5,188 lines) and `SportsShootDetailView.swift` (2,539) share 21 identically-named functions (~2,000 duplicated lines). Extract sync orchestration + lock handling + filter/sort into shared testable types; keep only layout per device. Port the cached-roster pattern (`FPSportsRosterView_iPad.swift:3468,4307`) to `SportsShootDetailView.swift:1281`, `SportsShootListView.swift:2760`, and cache `PoserStationView.filteredSubjects` (`:277`, evaluated ~8x/render)
- [ ] **SessionService refetch storm** (`SessionService.swift:90-101,222-240,726-748`): full-org fetch, no date bound, refetched per-subscriber (~10 views) on every realtime event; `recalculateSessionColorsForDate` = 1 UPDATE per session, each triggering more refetches. Debounce, share one fetch, batch color updates into a single RPC, bound query by date range. Also: cache ignores `includeUnpublished` (`:77-83`) → unpublished sessions leak to employees for up to 5 min
